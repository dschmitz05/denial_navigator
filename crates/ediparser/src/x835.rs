...
use crate::common::{
    detect_delimiters, opt, person_name, round2, one_or, split_segments, to_date, to_float,
    Delimiters, Segment,
};
use crate::schema::{
    Parsed835Response, ParsedClaim, ParsedClaimAdjustment, ParsedDenial, ParsedPaymentInfo,
    ParsedServiceLine, ParsedServiceLineDetail, ParsedTransactionMetadata,
};
use crate::x837::parse_837;
...
pub fn parse(content: &str) -> Result<Parsed835Response, String> {
    ...
    let segments = split_segments(content, &delims);
...
fn check_balance(claim: &ParsedClaim, warnings: &mut Vec<String>) {
    ...
    for detail in &claim.service_lines {
        ...
        let line_adjustment: f64 = detail
            .adjustments
            .iter()
            .map(|a| a.amount)
            .sum::<f64>();
...
fn is_denial(adjustment: &ParsedClaimAdjustment) -> bool {
    VALID_GROUP_CODES.contains(&adjustment.adjustment_group_code.as_str())
        && adjustment.reason_code.as_deref().is_some_and(|s| !s.is_empty())
