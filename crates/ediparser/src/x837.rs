...
use crate::common::{opt, person_name, to_date, to_float, one_or, round2, Segment};
use crate::schema::{ParsedClaim, ParsedServiceLine, ParsedServiceLineDetail};
...
impl Context {
    fn effective_patient(&self) -> (Option<String>, Option<String>, Option<String>) {
        if self.in_dependent_loop && (self.patient_name.is_some() || self.patient_id.is_some()) {
            let id = self.patient_id.clone().or_else(|| self.subscriber_id.clone());
            let dob = self.patient_dob.clone().or_else(|| self.subscriber_dob.clone());
