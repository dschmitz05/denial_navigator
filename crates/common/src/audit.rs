...
use crate::rbac::{client_ip_from, resolve_from};

/// Infrastructure, not PHI. Auditing these adds noise without recording a
/// single access to patient data.
const SKIP_EXACT: &[&str] = &["/", "/health", "/docs", "/redoc", "/openapi.json", "/favicon.ico"];
...
pub fn classify(method: &str, path: &str) -> (String, String, Option<String>) {
    ...
    if let Some((_, _, action)) =
        NAMED_ACTIONS.iter().find(|(m, p, _)| *m == method && *p == norm)
    {
    ...
    (format!("{verb}_{resource_type}"), resource_type, resource_id)
}

const LABEL_COLUMNS: &[(&str, &[&str])] = &[
    ("claim", &["claim_number", "patient_name"]),
    ("denial", &["claim_number", "patient_name", "cpt_code", "carc_code"]),
    (
        "appeal",
...
fn label_query(resource_type: &str) -> Option<&'static str> {
    match resource_type {
        ...
        "user" => Some(
            "SELECT username AS target_username FROM users WHERE id = $1::uuid",
        ),
        "knowledge_doc" => Some(
            "SELECT title AS document_title FROM knowledge_documents WHERE id = $1::uuid",
        ),
        _ => None,
    }
}
