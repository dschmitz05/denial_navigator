...
function workType(resolutionType) {
  return WORK_TYPES[resolutionType] || {
...
export default function Worklist() {
  ...
  const [feedback, setFeedback] = useState({ rating: 0, feedback_text: '' })
  ...
  const openItem = async (item) => {
    ...
    try {
      ...
      setDetail(resp.ok ? data : null)
      if (!resp.ok) setNotice({ error: true, text: data?.detail || 'Could not load this item' })
    } catch (err) {
      setNotice({ error: true, text: err.message })
    }
  }

  const updateOutcome = async (itemId, newStatus) => {
    setUpdating(true)
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error(data.detail || `Update failed (HTTP ${resp.status})`)
      }
      loadItems({ outcome_status: outcomeFilter, resolution_type: typeFilter })
      const refreshed = await resp.json()
      setSelected(prev => (prev ? { ...prev, outcome_status: refreshed.outcome_status } : prev))
    } catch (err) {
      setNotice({ error: true, text: err.message })
    }
    setUpdating(false)
  ...
  const submitFeedback = async (item, outcome) => {
  ...
  const closeOut = async (item, outcome) => {
    setUpdating(true)
  ...
  const meta = selected ? workType(selected.resolution_type) : null
