...
function ClaimDetails({ claim, onRefresh, refreshing }) {
...
export default function Claims() {
  ...
  const [details, setDetails] = useState({})   // claim.id -> full claim record
  ...
  const loadClaims = () => {
    ...
    fetch(`${API_BASE}/claims?${params}`)
      ...
      .then(data => {
        ...
        setDetailError(null)
        setLoading(false)
        if (expandedId && data.some(c => c.id === expandedId)) loadDetail(expandedId, true)
      })
      .catch(err => { console.error(err); setLoading(false) })
  }

  const loadDetail = (id, force = false) => {
    ...
    fetch(`${API_BASE}/claims/${id}`)
      ...
      .catch(err => setDetailError(err.message || 'Could not load claim details'))
      .finally(() => setDetailLoading(prev => (prev === id ? null : prev)))
  }

  const toggleExpand = (claim) => {
