...
type Outcome = { label: string; tone: 'ok' | 'blocked' | 'warn' | 'error' }
 ...
 */
export function actionHeadline(action: string | null | undefined, details: unknown): string {
...
export function describeResource(resourceType?: string | null): string {
...
export function parseDetails(details: unknown): AuditDetails {
  if (!details) return {}
 ...
 */
export function describeOutcome(details: unknown): Outcome | null {
...
const prettyRole = (r: unknown) => (r ? String(r).replace(/_/g, ' ') : r)
const prettyWork = (r: unknown) => (r ? String(r).replace(/_/g, ' ') : 'work')
 ...
 */
export function describeSubject(details: unknown): string {
  const d = parseDetails(details)
  if (d.claim_number) {
 ...
 */
export function describeDetails(action: string | null | undefined, details: unknown): string {
  const d = parseDetails(details)
...
export function technicalDetails(row: AuditRow): Array<[string, string]> {
