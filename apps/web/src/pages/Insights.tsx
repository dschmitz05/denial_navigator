...
type Analytics = {
  ...
  outcome_known_count: number
  success_rate?: number | null
  success_count?: number
  acceptance_rate?: number | null
  avg_rating?: number | null
  money?: { recovered?: number; still_open?: number; not_recovered?: number }
  by_payer?: MetricRow[]
  by_required_action?: MetricRow[]
  by_playbook?: MetricRow[]
  by_carc?: MetricRow[]
  trend?: MetricRow[]
}

const money = (v?: number | null) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(v || 0)
const pct = (v?: number | null) => (v === null || v === undefined ? '—' : `${Math.round(v * 100)}%`)
const rating = (v?: number | null) => (v === null || v === undefined ? '—' : `${v.toFixed(1)} / 5`)
...
export default function Insights() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/feedback/analytics`)
      ...
      .then((value: unknown) => setData(value as Analytics))
      ...
      <Section title="By payer" hint="A model that reads one payer badly is a different problem from one that is uniformly mediocre.">
        <Table
          ...
          rows={(data.by_payer || []).map(p => [
            ...
            `${p.paid_count}/${p.outcome_known}`, pct(p.success_rate as number), money(p.recovered_amount as number),
          ])}
        />
      </Section>
      ...
      <Section title="By recommended action" hint="Where the advice holds up, and where it does not.">
        <Table
          ...
          rows={(data.by_required_action || []).map(a => [
            String(a.required_action || '').replace(/_/g, ' '), a.feedback_count,
            `${a.paid_count}/${a.outcome_known}`, pct(a.success_rate as number), rating(a.avg_rating as number),
          ])}
        />
      </Section>

      <Section title="By applied playbook" hint="Outcome rates for approved deterministic playbooks; unlinked analyses remain visible as a comparison group.">
        <Table
          head={['Playbook', 'Rated', 'Paid', 'Success', 'Avg rating']}
          rows={(data.by_playbook || []).map(p => [
            p.playbook_name, p.feedback_count,
            `${p.paid_count}/${p.outcome_known}`, pct(p.success_rate as number), rating(p.avg_rating as number),
          ])}
        />
      </Section>
      ...
      <Section title="By denial reason" hint="The reason codes worth trusting the model on.">
        <Table
          ...
          rows={(data.by_carc || []).map(c => [
            c.carc_code, String(c.carc_description || '').slice(0, 60), c.feedback_count, pct(c.success_rate as number),
          ])}
        />
      </Section>
      ...
      <Section title="Over time" hint="A lifetime average cannot tell a model that improved from one that never worked.">
        <Table
          ...
          rows={(data.trend || []).map(t => [
            new Date(String(t.month)).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
            t.feedback_count, t.outcome_known, pct(t.success_rate as number), rating(t.avg_rating as number),
          ])}
        />
      </Section>
