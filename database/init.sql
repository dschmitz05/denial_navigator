...
-- CMS NCCI PTP/MUE reference edits (FB-20), for post-denial explanation.
CREATE TABLE ncci_ptp_edits (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), column_1_code VARCHAR(20) NOT NULL,
    column_2_code VARCHAR(20) NOT NULL, modifier_indicator SMALLINT NOT NULL CHECK (modifier_indicator IN (0,1,9)),
...
CREATE TABLE ncci_mue_edits (
    ...
    mue_value INTEGER NOT NULL CHECK (mue_value > 0), adjudication_indicator SMALLINT CHECK (adjudication_indicator IN (1,2,3)),
    effective_date DATE, termination_date DATE, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE (code, effective_date)
);
